...
(function (App) {
  ...
  function renderRegisterForm(panel) {
    ...
    panel.querySelector('#regBtn').addEventListener('click', () => {
      ...
      if (!/^\d{10}$/.test(phone)) {
        toast('เบอร์โทรศัพท์ต้องเป็นตัวเลขความยาว 10 หลัก', true);
        return;
      }

      if (!/^\d{13}$/.test(idCard)) {
        toast('เลขบัตรประชาชนต้องเป็นตัวเลขความยาว 13 หลัก', true);
        return;
      }
      ...
      state.users.push(u);
      state.currentUser = u;
      App.saveUsersToStorage(); // บันทึกอีเมล/รหัสผ่านไว้ถาวร เพื่อให้กลับมา login ได้อีกครั้งในภายหลัง
